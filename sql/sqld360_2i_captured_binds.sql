...
BEGIN
  ...
  LOOP
    ...
    IF i.datatype_string LIKE 'NUMBER%' OR i.datatype_string = 'DATE' THEN
        ...
        put(q'[        'Captured: '||TO_CHAR(last_captured, 'YYYY-MM-DD HH24:MI:SS')||', Value: '||value_string               ]');
    ...
    put(q'[       num_captures,                                                                                                                                      ]');
    put(q'[       null,                                                                                                                                              ]');
    put(q'[       'Value: '||value_string||' - Number of times captured: '||num_captures||' ('||TRUNC(100*RATIO_TO_REPORT(num_captures) OVER (),2)||'% of total)'    ]'); 
