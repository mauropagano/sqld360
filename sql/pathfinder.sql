...
DECLARE
  ...
  BEGIN
    ...
    IF (p_alter_session = 'BASELINE') THEN
      ...
      put('SELECT /* not me */ child_number child_number_, plan_hash_value, ''<tr class="main"><td align=right>'||l_test_id||'</td><td align=right></td><td>'||replace(p_alter_session,chr(39),chr(39)||chr(39))||'</td><td align=right>''||plan_hash_value||''</td><td align=right>''||TRUNC(elapsed_time/1e6,3)||''</td><td align=right>''||TRUNC(cpu_time/1e6,3)||''</td><td align=right>''||buffer_gets||''</td><td align=right>''||rows_processed||''</td><td><a href="'||l_spoolfile_name_p||'">plan</a></td><td><a href="'||l_spoolfile_name_vs||'">details</a></td></tr>'' html_row FROM v$sql WHERE sql_text like ''%'||l_unique_id||'%'' AND sql_text NOT LIKE ''%not me%'';');
    ELSE
      put('SELECT /* not me */ child_number child_number_, plan_hash_value, CASE WHEN plan_hash_value = ^^baseline_phv. THEN ''--'' ELSE NULL END skip_cfb, ''<tr class="main"><td align=right>'||l_test_id||'</td><td align=right></td><td>'||replace(p_alter_session,chr(39),chr(39)||chr(39))||'</td><td align=right>''||plan_hash_value||''</td><td align=right>''||TRUNC(elapsed_time/1e6,3)||''</td><td align=right>''||TRUNC(cpu_time/1e6,3)||''</td><td align=right>''||buffer_gets||''</td><td align=right>''||rows_processed||''</td><td><a href="'||l_spoolfile_name_p||'">plan</a></td><td><a href="'||l_spoolfile_name_vs||'">details</a></td></tr>'' html_row FROM v$sql WHERE sql_text like ''%'||l_unique_id||'%'' AND sql_text NOT LIKE ''%not me%'';'); 
    END IF;
    ...
    FOR i IN 1..4 LOOP
      ...
      put('SELECT /* not me */ CASE WHEN (plan_hash_value <> ^^plan_hash_value'||CASE i WHEN 1 THEN NULL ELSE '_rp'||(i-1) END||'. AND ''^^skip_cfb'' IS NULL) THEN NULL ELSE ''--'' END skip_execution_rp'||i||', child_number child_number_rp'||i||', plan_hash_value plan_hash_value_rp'||i||', ''<tr class="main"><td align=right>'||l_test_id||'</td><td align=right>'||l_test_id_rp_i||'</td><td>'||replace(p_alter_session,chr(39),chr(39)||chr(39))||'</td><td align=right>''||plan_hash_value||''</td><td align=right>''||TRUNC(elapsed_time/1e6,3)||''</td><td align=right>''||TRUNC(cpu_time/1e6,3)||''</td><td align=right>''||buffer_gets||''</td><td align=right>''||rows_processed||''</td><td><a href="'||l_spoolfile_name_rp_i_p||'">plan</a></td><td><a href="'||l_spoolfile_name_rp_i_vs||'">details</a></td></tr>'' html_row ');
