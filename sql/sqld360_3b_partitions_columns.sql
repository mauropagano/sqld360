...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      put('SELECT /*+ &&top_level_hints. */');
      put('       a.*, ');
      put('       CASE WHEN data_type = ''''NUMBER'''' THEN to_char(utl_raw.cast_to_number(a.low_value)) ');
      put('        WHEN data_type IN (''''VARCHAR2'''', ''''CHAR'''') THEN to_char(utl_raw.cast_to_varchar2(a.low_value)) ');
      put('        WHEN data_type IN (''''NVARCHAR2'''',''''NCHAR'''') THEN to_char(utl_raw.cast_to_nvarchar2(a.low_value)) ');
      put('        WHEN data_type = ''''BINARY_DOUBLE'''' THEN to_char(utl_raw.cast_to_binary_double(a.low_value)) ');
      put('        WHEN data_type = ''''BINARY_FLOAT'''' THEN to_char(utl_raw.cast_to_binary_float(a.low_value)) ');
      put('        WHEN data_type = ''''DATE'''' THEN rtrim( ');
      put('                    ltrim(to_char(100*(to_number(substr(a.low_value,1,2) ,''''XX'''')-100) + (to_number(substr(a.low_value,3,2) ,''''XX'''')-100),''''0000''''))||''''-''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,5,2) ,''''XX'''')  ,''''00''''))||''''-''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,7,2) ,''''XX'''')  ,''''00''''))||''''/''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,9,2) ,''''XX'''')-1,''''00''''))||'''':''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,11,2),''''XX'''')-1,''''00''''))||'''':''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,13,2),''''XX'''')-1,''''00''''))) ');
      put('        WHEN data_type LIKE ''''TIMESTAMP%'''' THEN rtrim( ');
      put('                    ltrim(to_char(100*(to_number(substr(a.low_value,1,2) ,''''XX'''')-100) + (to_number(substr(a.low_value,3,2) ,''''XX'''')-100),''''0000''''))||''''-''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,5,2) ,''''XX'''')  ,''''00''''))||''''-''''|| '); 
      put('                    ltrim(to_char(     to_number(substr(a.low_value,7,2) ,''''XX'''')  ,''''00''''))||''''/''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,9,2) ,''''XX'''')-1,''''00''''))||'''':''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,11,2),''''XX'''')-1,''''00''''))||'''':''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.low_value,13,2),''''XX'''')-1,''''00''''))||''''.''''|| ');
      put('                    to_number(substr(a.low_value,15,8),''''XXXXXXXX'''')) ');
      put('       END low_value_translated, ');
      put('       CASE WHEN data_type = ''''NUMBER'''' THEN to_char(utl_raw.cast_to_number(a.high_value)) ');
      put('        WHEN data_type IN (''''VARCHAR2'''', ''''CHAR'''') THEN to_char(utl_raw.cast_to_varchar2(a.high_value)) ');
      put('        WHEN data_type IN (''''NVARCHAR2'''',''''NCHAR'''') THEN to_char(utl_raw.cast_to_nvarchar2(a.high_value)) ');
      put('        WHEN data_type = ''''BINARY_DOUBLE'''' THEN to_char(utl_raw.cast_to_binary_double(a.high_value)) ');
      put('        WHEN data_type = ''''BINARY_FLOAT'''' THEN to_char(utl_raw.cast_to_binary_float(a.high_value)) ');
      put('        WHEN data_type = ''''DATE'''' THEN rtrim( ');
      put('                    ltrim(to_char(100*(to_number(substr(a.high_value,1,2) ,''''XX'''')-100) + (to_number(substr(a.high_value,3,2) ,''''XX'''')-100),''''0000''''))||''''-''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.high_value,5,2) ,''''XX'''')  ,''''00''''))||''''-''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.high_value,7,2) ,''''XX'''')  ,''''00''''))||''''/''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.high_value,9,2) ,''''XX'''')-1,''''00''''))||'''':''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.high_value,11,2),''''XX'''')-1,''''00''''))||'''':''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.high_value,13,2),''''XX'''')-1,''''00''''))) ');
      put('        WHEN data_type LIKE ''''TIMESTAMP%'''' THEN rtrim( ');
      put('                    ltrim(to_char(100*(to_number(substr(a.high_value,1,2) ,''''XX'''')-100) + (to_number(substr(a.high_value,3,2) ,''''XX'''')-100),''''0000''''))||''''-''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.high_value,5,2) ,''''XX'''')  ,''''00''''))||''''-''''|| '); 
      put('                    ltrim(to_char(     to_number(substr(a.high_value,7,2) ,''''XX'''')  ,''''00''''))||''''/''''|| ');
      ...
      put('                    ltrim(to_char(     to_number(substr(a.high_value,11,2),''''XX'''')-1,''''00''''))||'''':''''|| ');
      put('                    ltrim(to_char(     to_number(substr(a.high_value,13,2),''''XX'''')-1,''''00''''))||''''.''''|| ');
      put('                    to_number(substr(a.high_value,15,8),''''XXXXXXXX'''')) ');
      put('       END high_value_translated ');
      put('  FROM dba_part_col_statistics a,');
      put('       dba_tab_cols b');
      put(' WHERE a.owner = '''''||j.table_owner||''''''); 
      put('   AND a.table_name = '''''||j.table_name||'''''');
